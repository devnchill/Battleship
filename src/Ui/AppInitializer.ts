...
class AppInitializer {
  ...
  public setupFirstPage() {
    ...
    const svgIcon = new BuildElement("svg", "", "", "", [], "");
    svgIcon.element.setAttribute("viewBox", "0 0 105 105");
    svgIcon.element.setAttribute("xmlns", "http://www.w3.org/2000/svg");

    const path = new BuildElement(
      "path",
      "", // No src or alt, just the attributes
      "",
      "",
      [],
      "",
    );
    path.element.setAttribute("fill-rule", "evenodd");
    path.element.setAttribute("clip-rule", "evenodd");
    ...
    path.element.setAttribute("fill", "#24292f");

    svgIcon.element.appendChild(path.element);

    githubLink.element.appendChild(svgIcon.element);
